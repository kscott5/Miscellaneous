...
function routeAllPaths(args, parent) {
    ...
    node.forEach(function(nodeVale, nodeId, thisMap) {
        ...
        var userInput = startNodeId.concat('-', travelPath).replace(/,/g,'-'); // Build train specific user input
        travelPath = parent.routePath(userInput); // Now route the complete path
        
        // Save valid data to check later
        if(travelPath.valid /* add args.condition statement here */ ) {                
...
function routeTravelPathsWithSameNodes(args, parent) {
    ...
    if(results.travelCost.length > 0) {     
        // Save the current maximum length of results. 
        // This is important because results size could grow
        var maxLength = results.output.length; 
        
        for(var i=0; i<maxLength; i++) { // Loop over results using current maximum length
            var route = results.output[i];
            var searching = false;
            
            do { // search by suffixing the route with original path
                route = route.concat(results.output[i].substr(1, results.output[i].length));
                var result = routeCompletePath(route, parent);
                
                // Save search results 
                if(searching = (result.valid && args.condition({travelCost: result.travelCost}))) {
                     results.output.push(result.output);
                     results.travelCost.push(result.travelCost);   
                }
            } while(searching)
        } // end for loop
