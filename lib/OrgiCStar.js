...
*/
function findSingleTravelPath(route, parent) {
...
function findAllTravelPathsRecursively(args) {
    ...
    node.forEach(function(childeNodeCost,childNodeId,thisMap) {
        ...
        if(childNodeId == args.targetNodeId) {
            var travelResults = findSingleTravelPath(travelPath, args.parent);
            if(travelResults.valid && (passedConditionCheck = args.condition({travelStops: travelResults.travelStops}))) {
...
OrgiCStar.prototype.routePath = function(args) {
    if(typeof args == 'string')
        return findSingleTravelPath(args, this);
