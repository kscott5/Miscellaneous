...
class OrgiCStar {
    ...
    */
    routePath(args) {
        const self = this; // scope block used for private functions 
        
        if(typeof args == 'string')
            return findTravelPath.call(this,args);
        
        if(!args.startNodeId || !args.endNodeId)
            return null;
            
        return findAllTravelPathsByNodes.call(this,args);
        
        //***************************************************************************************
        //* Below are list of functions that are used privately to OrgiCStar.routePath() method
        //***************************************************************************************
        ...
        */
        function findTravelPath(route) {
            ...
            for(var i=0; i<inputTransformed.length-1; i++) {            
                ...
                var node = self.route.graph.get(startNodeId);
        ...
        */
        function findAllTravelPathsByNodes(args) {    
            ...
            node.forEach(function(childNodeCost,childNodeId,thisMap){
                var travelResults = findAllTravelPathsRecursively.call(self,{
            ...
            if(args.shortestRoute && typeof results.output != 'string' && results.output.length > 0) {        
                results.consolidated.sort(
                    function(left,right){
                        if(parseInt(left.travelCost)<parseInt(right.travelCost))
                            return -1;
                        if(parseInt(left.travelCost)>parseInt(right.travelCost))
                            return 1;
        ...
        */
        function findAllTravelPathsRecursively(args) {
            var results = [];
            var node = self.route.graph.get(args.startNodeId);
            ...
            node.forEach(function(childeNodeCost,childNodeId,thisMap) {
                ...
                if(childNodeId == args.targetNodeId) {
                    var travelResults = findTravelPath.call(self,travelPath);
                ...
                (!args.shortestRoute && (passedConditionCheck || matches.length < 2))) {
                    var tmp = findAllTravelPathsRecursively.call(self,{
                        shortestRoute: args.shortestRoute,
